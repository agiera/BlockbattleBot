 ...
 *   1 3 5 7 ...
 *
 * Internally, actions are stored as a vector of individual agent
 * actions. Based on the SCALAR_ACTION variable in dsn_common.h either
 * the scalar (joint) action or the individual actions are returned.
 * 
...
// scalar actions, 0 = int* is used for actions, 1 = int 
#define SCALAR_ACTION     1

typedef int*   ActionDim;            // actions of different agent 
...
#define NR_ACTIONS         3 // number of actions
#define START_ENERGY_LEVEL 3 // starting energy level for a target
#define NR_STATES          (NR_AGENTS/2-1) // number of target positions

// Global variables
Observation         g_o;
ActionDim           g_a;         // internally actions are stored as int*
ActionDim           g_actions;   // temporary variable 
Reward_observation  g_ro;

#if SCALAR_ACTION                // define conversion between scalars and
ActionDim splitAction( int a );  // multi-dimensional actions

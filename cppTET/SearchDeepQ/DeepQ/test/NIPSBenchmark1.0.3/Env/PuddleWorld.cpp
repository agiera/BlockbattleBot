...
#define STATE_DIM 2
    ...
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",0.0,1.0); // min max for dim 1 can be written here
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",0.0,1.0); // min max for dim 2 can be written here
    strcat(Task_spec,temp);
    sprintf(temp,":1_[i]");
    strcat(Task_spec,temp);
    sprintf(temp,"_[%d,%d]",0,3); // min max for actions can be written here
    strcat(Task_spec,temp);
    sprintf(temp,":%f,%f",-1.0 - 400*PUDDLERADIUS,-1.0); // Rmin, Rmax
    strcat(Task_spec,temp);
    sprintf(temp,"\0");
    strcat(Task_spec,temp);
    return Task_spec;

...
#ifndef ENV_STATE
void env_get_state(State_key* state)
{
...
{
	printf("\nError:: env_set_state must be defined in your enviornment c file before you can call RL_set_state\n\n");
        exit(1);
}
#endif
...
{
	printf("\nError:: env_get_random_seed must be defined in your enviornment c file before you can call RL_get_random_seed\n\n");
	exit(1);        
	Random_seed_key rsk;
	return rsk;
}
...
{
	printf("\nError:: env_set_random must be defined in your enviornment c file before you can call RL_set_random_seed\n\n");
	exit(1);
}
#endif
...
#ifdef PIPES
	task_spec = env_init(path);
	agent_init(task_spec,path);
#else
	task_spec = env_init();
...
{
    return (Reward)(total_reward/num_steps);
}

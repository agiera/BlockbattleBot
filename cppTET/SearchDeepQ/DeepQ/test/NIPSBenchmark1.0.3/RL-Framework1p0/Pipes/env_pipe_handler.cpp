...
int main(int argc, char** argv)
{
	...
	if (argc != 3)
	{
		printf("Error: environment needs to know the name of the FIFO.\n");
		exit(1);
	}
    ...
    {   
        ...
        else if(strcmp(pipe_env_buf,"getname") == 0)
        {
        	pipe_env_get_name();
        }
...
{
#ifdef ENV_NAME
	env_name = env_get_name();
	pipe_write(pipe_out_stream, env_name);
#else
	pipe_write(pipe_out_stream,"NOTIMPLEMENTED");
#endif
}

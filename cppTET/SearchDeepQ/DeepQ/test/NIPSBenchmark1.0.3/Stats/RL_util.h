...
//INIT ROUTINE USED FOR NIPS WORKSHOP
void initStatsBenchmark(char* envName, char* agentName, int num_episodes)
{
    char temp1[500];

    strcpy(temp1,"Data/");
    strcat(temp1,envName);
    strcat(temp1,"_");
    strcat(temp1,agentName);
    strcat(temp1,".dat");
    printf("Saving data to ");
    printf(temp1);
    printf("...\n");
    f1 = fopen(temp1,"w");
    strcpy(temp1,envName);
    strcat(temp1,"\n");
    strcat(temp1,agentName);
    strcat(temp1,"\n");
    #ifdef PIPES
    strcat(temp1,"Pipes\n");
    #else
    strcat(temp1,"No pipes\n");
    #endif
    strcat(temp1,"Version 1.0.3r\n");
	fputs(temp1,f1);
...
//COLLECTION ROUTINE USED FOR NIPS
void collectStatsBenchmark(int n, int index)
{
	util_endTimer(&t_end_util);
	episodeTime += (double)(t_end_util.tv_sec - t_start_util.tv_sec) + (double)(t_end_util.tv_usec - t_start_util.tv_usec)/(1000000.0);
	util_startTimer(&t_start_util);
	cummulativeReward += RL_return();
	numberOfSteps += RL_num_steps();
	
	if((index+1)% n == 0)
	{
		cummulativeReward = cummulativeReward/n;
		double avgNumberOfSteps = (double)numberOfSteps;
		avgNumberOfSteps = avgNumberOfSteps/n;
		episodeTime = episodeTime/n;
	
		sprintf(buf,"%lf %lf %lf\n", episodeTime, cummulativeReward, avgNumberOfSteps);
		fputs(buf,f1);
		cummulativeReward = 0;
		numberOfSteps = 0;
		episodeTime = 0;
...
//DeInit ROUTINE USED FOR NIPS
void endStatsBenchmark()
{
    fclose(f1);
}

...
void initStats(int num_episodes)
{
    ...
    strcpy(temp1,"Data/episodeRewardChallenge");
    strcat(temp1,temp2);
    f4 = fopen(temp1,"w");
    
    numberSteps = new int[num_episodes];
    episodeRewards = list<double>(numEpisodesAvg, 0);
...
{	
    ...
    episodeRewards.push_back(RL_return());
    episodeRewards.pop_front();
    episodeReward = 0;
    for(list<double>::iterator it = episodeRewards.begin();
    	it != episodeRewards.end(); it++)
    {
    	episodeReward += *it;
    }
    
    sprintf(buf,"%lf\n", episodeReward/numEpisodesAvg);
    fputs(buf,f4);	
...
void endStats(int num_episodes)
{
    ...
    printf("--\n");
    printf("The final cummulative reward is = %f\n",endCumReward);        
    printf("The final average episode reward is = %f for the last %d episodes\n",episodeReward, numEpisodesAvg);
    printf("The final average reward is = %f\n",endAverageReward);
	printf("-------------------------------------------------------\n");
	delete [] numberStepsData; numberStepsData = 0;

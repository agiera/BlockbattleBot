 ...
 * 
 *  Grouped version: consists of 19 variables. One is related to the number of rolls
 ...
 *  represent the current values of the dice, grouped so that the most numerous
 *  values are placed first (e.g., if we roll $<2,6,6,2,4>$, the five variables
 *  assume the following values: $<2,2,6,6,4>$ ).
 * 
...
Task_specification env_init()
{
    ...
    sprintf(temp,"_[i,i,i,i,i,i,i,i,i,i,i,i,i,i,i,i,i,i,i]"); // number of 'i's depends on number of dimensions
    strcat(Task_spec,temp);
    ...
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",  1.0,  6.0);  // dice1
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",  1.0,  6.0);  // dice2
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",  1.0,  6.0);  // dice3
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",  1.0,  6.0);  // dice4
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",  1.0,  6.0);  // dice5
...
void setObservation()
{
    ...
    unsigned int grouped_dice[NUMBER_OF_DICE];
    for (unsigned int i = 0; i < NUMBER_OF_DICE; i++)
    {
        grouped_dice[i] = gYahtzeeEnv.mDice[i];
    }

    for (unsigned int i = 0; i < NUMBER_OF_DICE; i++)
    {
        unsigned int j = i;
        unsigned int dice = grouped_dice[j];
        while (j > 0 && results[grouped_dice[j-1]-1] < results[dice-1])
        {
            grouped_dice[j] = grouped_dice[j-1];
            j--;
        }
        grouped_dice[j] = dice;
    }
    ...
    for (unsigned int i = 0; i < NUMBER_OF_DICE; i++)
    {
        gYahtzeeState.mState[14+i] = grouped_dice[i];
    }
}
...
void printState()
{
    cout << "*** Yahtzee Grouped Full Environment ***" << endl;

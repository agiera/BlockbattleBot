...
void agent_end(Reward r)
{
	...
	if(agent[0]->episodes != 0 && agent[0]->episodes % 1000000 == 0)
	{
		char buf[100];
		sprintf(buf, "agent_0_%010d.txt", agent[0]->episodes);
		string file = Benchmark::Instance()->GetOutputDir() + buf;
 		SaveAgent(*agent[0], file.c_str());
	}
}
...
{
	Benchmark::Instance()->EndRun();
	string file = Benchmark::Instance()->GetOutputDir() + "agent.txt";
	SaveAgent(*agent[0], file.c_str());
	delete agent[0];
	agent[0] = NULL;
}

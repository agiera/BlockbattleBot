 ...
 * 
 *  Score version: consists of 14 variables. One is related to the number of rolls
 *  performed in the current round. The other 13 are associated to the 13
 *  categories, and each one contains the score that the player would get by taking
 *  it. If the category has been already used, the respective variable is set to -1.
 *  The size of this state space is about 0,44x10^11.
 * 
 *  Full version: the available configurations are the same as in the
 *  original game.
 *  
 *  State:   14 discrete variables
 *  Actions:  1 discrete variable
 *
 */

#include <iostream>
#include "YahtzeeScoreFull.h"
...
Task_specification env_init()
{
    ...
    sprintf(temp,"_[i,i,i,i,i,i,i,i,i,i,i,i,i,i]"); // number of 'i's depends on number of dimensions
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]",  0.0,  3.0);  // reroll
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0,  5.0);  // ones
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 10.0);  // twos
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 15.0);  // threes
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 20.0);  // fours
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 25.0);  // fives
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 30.0);  // sixes
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 30.0);  // tris
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 30.0);  // poker
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 25.0);  // full
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 30.0);  // small
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 40.0);  // large
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 30.0);  // chance
    strcat(Task_spec,temp);
    sprintf(temp,"_[%f,%f]", -1.0, 40.0);  // yahtzee
    strcat(Task_spec,temp);
...
void setObservation()
{
    ...
    // set to -1 all the configuration not available
    for (unsigned int i = 0; i < NUMBER_OF_COMBO; i++)
    {
    ...
    // update the value of the chance
    if (-1 != gYahtzeeState.mState[STATE_CHANCE])
        gYahtzeeState.mState[STATE_CHANCE] = total_sum;

    // update the values of the state variables
    if (-1 != gYahtzeeState.mState[STATE_ONES])
        gYahtzeeState.mState[STATE_ONES]   = results[0];
    if (-1 != gYahtzeeState.mState[STATE_TWOS])
        gYahtzeeState.mState[STATE_TWOS]   = 2*results[1];
    if (-1 != gYahtzeeState.mState[STATE_THREES])
        gYahtzeeState.mState[STATE_THREES] = 3*results[2];
    if (-1 != gYahtzeeState.mState[STATE_FOURS])
        gYahtzeeState.mState[STATE_FOURS]  = 4*results[3];
    if (-1 != gYahtzeeState.mState[STATE_FIVES])
        gYahtzeeState.mState[STATE_FIVES]  = 5*results[4];
    if (-1 != gYahtzeeState.mState[STATE_SIXES])
        gYahtzeeState.mState[STATE_SIXES]  = 6*results[5];

    if (-1 != gYahtzeeState.mState[STATE_TRIS])
        gYahtzeeState.mState[STATE_TRIS]    = 0;
    if (-1 != gYahtzeeState.mState[STATE_POKER])
        gYahtzeeState.mState[STATE_POKER]   = 0;
    if (-1 != gYahtzeeState.mState[STATE_FULL])
        gYahtzeeState.mState[STATE_FULL]    = 0;
    if (-1 != gYahtzeeState.mState[STATE_SMALL])
        gYahtzeeState.mState[STATE_SMALL]   = 0;
    if (-1 != gYahtzeeState.mState[STATE_LARGE])
        gYahtzeeState.mState[STATE_LARGE]   = 0;
    if (-1 != gYahtzeeState.mState[STATE_YAHTZEE])
        gYahtzeeState.mState[STATE_YAHTZEE] = 0;
    ...
    for (unsigned int i = 0; i < 6; i++)
    {
        if (3 == results[i])
        {
            // if there are exactly three dice with the same number, the tris
            // value is the sum of all the values of the dice
            if (-1 != gYahtzeeState.mState[STATE_TRIS])
                gYahtzeeState.mState[STATE_TRIS] = total_sum;
            ...
            for (unsigned int j = 0; j < 6; j++)
            {
                ...
                {
                    if (-1 != gYahtzeeState.mState[STATE_FULL])
                        gYahtzeeState.mState[STATE_FULL] = FULL_VALUE;
                }
            }
        }
    }
    ...
    for (int i = 0; i < 6; i++)
    {
        if (4 == results[i])
        {
            // if there are exactly three dice with the same number, the poker
            // value is the sum of all the values of the dice
            if (-1 != gYahtzeeState.mState[STATE_POKER])
                gYahtzeeState.mState[STATE_POKER] = total_sum;
        }
    }

    // compute the yahtzee value
    for (int i = 0; i < 6; i++)
    {
        if (5 == results[i])
        {
            if (-1 != gYahtzeeState.mState[STATE_YAHTZEE])
                gYahtzeeState.mState[STATE_YAHTZEE] = YAHTZEE_VALUE;
        }
    }

    // compute the small straight value
    if (
        1 == results[0] && 1 == results[1] && 1 == results[2] && 1 == results[3]
        ||
        1 == results[1] && 1 == results[2] && 1 == results[3] && 1 == results[4]
    ...
    {
        if (-1 != gYahtzeeState.mState[STATE_SMALL])
            gYahtzeeState.mState[STATE_SMALL] = SMALL_VALUE;
    }

    // compute the large straight value
    if (
        1 == results[0] && 1 == results[1] && 1 == results[2] && 1 == results[3] && 1 == results[4]
        ||
        1 == results[1] && 1 == results[2] && 1 == results[3] && 1 == results[4] && 1 == results[5]
    ...
    {
        if (-1 != gYahtzeeState.mState[STATE_LARGE])
            gYahtzeeState.mState[STATE_LARGE] = LARGE_VALUE;
    }
}

void printState()
{
    cout << "*** Yahtzee Score Full Environment ***" << endl;

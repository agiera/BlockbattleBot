...
class Field:
    ...
    def clear_line(self, i):
        for j in range(i, 1, -1): # third argument is decrement
        ...
        for atom in move:
            if atom == 'left':
                ...
                if not self.piece_fits(piece._rotations[rotation]):
                    new_field = np.copy(self.field).astype(int)
                    for (x, y) in piece._rotations[rotation]:
                            # new_field[y, x+1] = 2 + new_field[y, x+1]
                        # with open('broken-boards.txt', 'a') as out:
                            # out.write('left: '+ str(new_field) + '\n')
                        return None, False
            elif atom == 'right':
                piece._rotations = [[(x + 1, y) for (x, y) in rot] for rot in piece._rotations]
                if not self.piece_fits(piece._rotations[rotation]):
                ...
                if not self.piece_fits(piece._rotations[rotation]):
                    new_field = np.copy(self.field).astype(int)
                    for (x, y) in piece._rotations[(rotation + 1) % 4]:
                        return None, False
            elif atom == 'turnright':
                rotation = (rotation + 1) % 4
                if not self.piece_fits(piece._rotations[rotation]):
    ...
    def project_piece_down(self, cells):
        fall = True
        while self.piece_fits([(x, y+1) for (x, y) in cells]):
